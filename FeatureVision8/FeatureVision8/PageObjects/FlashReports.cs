...
{
    public class FlashReports
    {
        #region Private Variables
        private IWebDriver flashReports;
        ...
        public FlashReports(IWebDriver driver, ExtentTest testReturn)
        {
            // TODO: Complete member initialization
            this.flashReports = driver;
            test = testReturn;
        ...
        {
            get { return this.flashReports; }
            set { this.flashReports = value; }
        }

        ///<summary> Verify Sub Menu </summary>
        ///<param name="SubMenu">Sub Menu loaded</param>
        ///<returns></returns>
        public FlashReports VerifySubMenuAndSelect(string SubName)
        {
            Assert.IsTrue(driver._isElementPresent("xpath", "//h1[text()='FlashReports']"), "FlashReports Screen not present.");
            Assert.IsTrue(driver._isElementPresent("xpath", "//div[@class='title cursorpointer']/a[text()='" + SubName + "']"), SubName + " link not present.");
            Assert.IsTrue(driver._isElementPresent("xpath", "//div[@class='report-media']/i[contains(@class, '" + SubName.ToLower().Replace("sov","").Replace(" ","-") + "')]"), SubName + " icon not present.");
            //Assert.IsTrue(driver._isElementPresent("xpath", "//div[@class='report-content']/div[contains(@class, 'submenudescription')]"), "Ads And SOV description not present.");
            ...
            IList<IWebElement> SubMenuCount = driver._findElements("xpath", "//div[@class='title cursorpointer']/a");
            for (int i = 0; i < SubMenuCount.Count; i++)
            {
                if (SubName.Contains(SubMenuCount[i].Text))
                {
                    //Console.WriteLine("\n" + SubMenuCount[i].Text);
                    driver._click("xpath", "//div[@class='title cursorpointer']/a[text()='" + SubName + "']");
                    break;
                }
            }
            ...
            Results.WriteStatus(test, "Pass", "Verified, Sub Menu & Select Option");
            return new FlashReports(driver, test);
        }

        ///<summary> Verify Ads And SOV Screen </summary>
        ///<param></param>
        ///<returns></returns>
        public FlashReports VerifyAdsAndSOVScreen(bool icon=false,bool Cddl=false, bool Mddl=false)
        {
            Assert.IsTrue(driver._isElementPresent("xpath", "//div[@class='iscsidebar']"), "Left Navigation not found on Page.");

            Assert.IsTrue(driver._isElementPresent("xpath", "//*[@class='page-header']"), "Page Header text not present.");
            Assert.AreEqual("Ads & SOV", driver._getText("xpath", "//*[@class='page-header']"), "Page Header text does not match.");

            if(icon == true)
            {
                Assert.IsTrue(driver._isElementPresent("xpath", "//*[@class='tdchartoption icon-promo-cta-export headerIcon ng-scope']"), "Export icon not present.");
            }
            if (Cddl == true)
            {
                Assert.IsTrue(driver._isElementPresent("xpath", "//*[@id='dvFilterField1001']"), "Country DDL not present.");
            }
            if (Mddl == true)
            {
                Assert.IsTrue(driver._isElementPresent("xpath", "//*[@id='dvFilterField1002']"), "Media DDL not present.");
            }

            Assert.IsTrue(driver._isElementPresent("xpath", "//input[@id='txtCalendar']"), "Calendar textbox not present.");
            Assert.IsTrue(driver._isElementPresent("xpath", "//span[@class='fa fa-calendar spexpand spexpandRecentAdds']"), "Calendar icon not present.");

            Assert.IsTrue(driver._isElementPresent("xpath", "//div[@class='customScrollBar']"), "Thumbnail Images section not present.");

            Assert.IsTrue(driver._isElementPresent("xpath", "//div[@class='thumbnailDiv_text']"), "Images text not present.");
            Assert.IsTrue(driver._isElementPresent("xpath", "//div[@class='thumbnailDiv_images']"), "Images not present.");
            ...
            Results.WriteStatus(test, "Pass", "Verified, Ads And SOV Screen");
            return new FlashReports(driver, test);
        }
        ...
        ///<returns></returns>
        public FlashReports SelectTabOnRetailerActivity(string tabName = "")
        {
        ...
        ///<returns></returns>
        public FlashReports VerifyExportMenuAndSelectOption(string optionName = "")
        {
            ...
            Assert.IsTrue(driver._isElementPresent("xpath", "//div[@class='tdchartoption icon-promo-cta-export headerIcon ng-scope']"), "Export Button not present.");

            string[] exportOptionList = new string[] { "Download EXCEL", "Download PowerPoint" };
            Actions action = new Actions(driver);
            IWebElement exportButton = null;
            driver._click("xpath", "//div[@class='tdchartoption icon-promo-cta-export headerIcon ng-scope']");
   //         Assert.IsTrue(driver._waitForElement("xpath", "//div[@id='" + chartHeaderId + "']//ul[@data-headeroption-id='1']//a"), "Export DDL not present.");
            ...
            {
                Assert.IsTrue(driver._isElementPresent("xpath", "//ul[@id='ulOptionImage']//li/a[text()='" + exportOption + "']"), "'" + exportOption + "' Option not present");
                action.MoveToElement(driver.FindElement(By.XPath("//ul[@id='ulOptionImage']//li/a[text()='" + exportOption + "']"))).MoveByOffset(2, 1).Perform();
                if (exportOption.ToLower().Equals(optionName.ToLower()))
                    exportButton = driver.FindElement(By.XPath("//ul[@id='ulOptionImage']//li/a[text()='" + exportOption + "']"));
            }
        ...
        ///<returns></returns>
        public FlashReports VerifyAdsByChannel_ParentRetailerSection(bool channel = true)
        {
        ...
        ///</summary>
        ///<returns></returns>
        public FlashReports VerifyHelpIconOnAdsByChannelOrParentRetailerSection()
        {
            string chartHeaderId = "";
            if (driver._isElementPresent("xpath", "//div[@id='FlyerColumnChannelCustChartHeader']"))
                chartHeaderId = "FlyerColumnChannelCustChartHeader";
        ...
        ///<returns></returns>
        public FlashReports VerifyWindowIconOnAdsByChannelOrParentRetailerSection(bool reset = false, string selectOption = "")
        {
            string chartHeaderId = "";
            if (driver._isElementPresent("xpath", "//div[@id='FlyerColumnChannelCustChartHeader']"))
                chartHeaderId = "FlyerColumnChannelCustChartHeader";
        ...
        ///</summary>
        ///<returns></returns>
        public FlashReports VerifyWhenColumnIsDrilledDownOn()
        {
            string chartId = "";
            if (driver._isElementPresent("xpath", "//div[@id='FlyerColumnChannelCustChartHeader']"))
                chartId = "FlyerColumnChannelCustChart0chart";
        ...
        ///</summary>
        ///<returns></returns>
        public FlashReports VerifyExportMenuAndSelectOptionOnPagesByChannel_ParentRetailer(string optionName = "")
        {
            string chartHeaderId = "";
        ...
        ///<returns></returns>
        public FlashReports VerifyPagesByChannel_ParentRetailerSection(bool channel = true)
        {
            Assert.IsTrue(driver._isElementPresent("xpath", "//div[@class='tab-pane ng-scope active']//li[text()='Channel']"), "Channel radio button not present.");
            Assert.IsTrue(driver._isElementPresent("xpath", "//div[@class='tab-pane ng-scope active']//li[text()='Parent Retailer']"), "Parent Retailer radio button not present.");
            driver._scrollintoViewElement("xpath", "//div[@class='tab-pane ng-scope active']//li[text()='Channel']");
        ...
        ///</summary>
        ///<returns></returns>
        public FlashReports VerifyHelpIconOnPagesByChannelOrParentRetailerSection()
        {
            string chartHeaderId = "";
            if (driver._isElementPresent("xpath", "//div[@id='PageColumnChannelCustChartHeader']"))
                chartHeaderId = "PageColumnChannelCustChartHeader";
        ...
        ///<param name="selectOption">Option to be selected</param>
        ///<returns></returns>
        public FlashReports VerifyWindowIconOnPagesByChannelOrParentRetailerSection(bool reset = false, string selectOption = "")
        {
            string chartHeaderId = "";
            if (driver._isElementPresent("xpath", "//div[@id='PageColumnChannelCustChartHeader']"))
                chartHeaderId = "PageColumnChannelCustChartHeader";
        ...
        ///</summary>
        ///<returns></returns>
        public FlashReports VerifyWhenColumnIsDrilledDownOnFromPagesByChannel_ParentRetailer()
        {
            string chartId = "";
            if (driver._isElementPresent("xpath", "//div[@id='PageColumnChannelCustChartHeader']"))
                chartId = "PageColumnChannelCustChart0chart";
        ...
        ///<returns></returns>
        public FlashReports VerifyAdDetailsPopup(string headerCol, bool clickCancel = false)
        {
        ...
        ///</summary>
        ///<returns></returns>
        public FlashReports VerifyAds_PagesByWeekSection(bool adByWeek = true)
        {
        ...
        ///<returns></returns>
        public FlashReports VerifyEvents_ThemesByWeekSection(bool eventsByWeek = true)
        {
        ...
        ///</summary>
        ///<returns></returns>
        public FlashReports VerifyHelpIconOnAds_PagesByWeekSection()
        {
            string chartHeaderId = "";
        ...
        ///</summary>
        ///<returns></returns>
        public FlashReports VerifyHelpIconOnEvents_ThemesByWeekSection()
        {
